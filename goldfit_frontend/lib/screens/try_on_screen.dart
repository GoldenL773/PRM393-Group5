...
class _TryOnScreenState extends State<TryOnScreen> {
  ...
  /// Simulates realistic fitting generation with 2-second delay
  /// Requirements: 8.5
  ...
  Widget _buildClothingImage(ClothingItem item) {
    // If item has asset path, use Image.asset
  ...
  Widget _buildBasePhotoPlaceholder(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 8),
        Text(
          'Select clothing items below to try them on',
          style: Theme.of(context).textTheme.bodySmall?.copyWith(
            color: GoldFitTheme.textLight,
          ),
...
class _ClothingSelectorBottomSheetState extends State<_ClothingSelectorBottomSheet> {
          ...
          Padding(
            ...
            child: Row(
              ...
              children: [
                ...
                Row(
                  children: [
                    if (widget.appState.selectedItemIds.isNotEmpty)
                      TextButton(
                        onPressed: () {
                          widget.appState.clearTryOnSelection();
                        },
  ...
  Widget _buildClothingGrid() {
    ...
    final items = _selectedCategory != null
        ? widget.appState.dataProvider.getItemsByCategory(_selectedCategory!)
        : widget.appState.allItems;
    ...
    return GridView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final isSelected = widget.appState.selectedItemIds.contains(item.id);
        ...
        return _ClothingItemSelector(
          ...
          onTap: () {
            if (isSelected) {
              widget.appState.deselectItemForTryOn(item.id);
            } else {
              widget.appState.selectItemForTryOn(item.id);
            }
          },
        );
      },
  ...
  Widget _buildDoneButton(BuildContext context) {
    final selectedCount = widget.appState.selectedItemIds.length;
