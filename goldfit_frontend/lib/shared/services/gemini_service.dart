...
class GeminiService {
  ...
  static String _getInitialTextKey() {
    final textKey = dotenv.env['GEMINI_API_KEY_TEXT'];
    if (textKey != null && textKey.trim().isNotEmpty) return textKey.trim();
    final mainKey = dotenv.env['GEMINI_API_KEY'];
    if (mainKey != null && mainKey.trim().isNotEmpty) return mainKey.trim();
    return 'PLACEHOLDER_GEMINI_KEY';
  }

  GeminiService()
    : _textModel = GenerativeModel(
        model: 'gemini-2.5-flash',
        apiKey: _getInitialTextKey(),
      ),
  ...
  String get _apiKey {
    ...
    if (key == null || key.trim().isEmpty) return 'PLACEHOLDER_GEMINI_KEY';
    return key.trim();
  }

  String get _textApiKey {
    final key = dotenv.env['GEMINI_API_KEY_TEXT'];
    if (key == null || key.trim().isEmpty) return _apiKey;
    return key.trim();
  }

  /// Helper for sending raw HTTP requests with responseModalities
  Future<String?> _generateImageViaRest(
    String model,
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (candidates != null && candidates.isNotEmpty) {
          ...
          if (parts != null && parts.isNotEmpty) {
            ...
            if (inlineData != null) {
              // ignore: avoid_print
              print(
                'DEBUG: Gemini API successfully returned an image inlineData.',
              );
        ...
        // ignore: avoid_print
        print(
          'DEBUG: Gemini API Error: ${response.statusCode} - ${response.body}',
        );
  ...
  Future<String?> generatePoseVariation(
    ...
    try {
      ...
      final bytes = await file.readAsBytes();
      return generatePoseVariationForBase64(
        base64Encode(bytes),
        poseInstruction,
      );
    } catch (e) {
  ...
  Future<String?> generatePoseVariationForBase64(
    String base64Image,
    String poseInstruction,
  ) async {
    try {
        ...
        "contents": [
          {
            "parts": [
              ...
              {
                "text":
                    "You are an expert fashion photographer AI. Regenerate this image with a different perspective: $poseInstruction. Keep the person, clothing, and background style identical. Return ONLY the final image.",
              },
            ],
          },
        ],
  ...
  Future<String?> removeBackground(String imagePath) async {
    try {
      ...
      final removeBgKey =
          dotenv.env['REMOVE_BG_API_KEY'] ?? 'vhooNUm1h6akn3RmiSG8eMfP';
...
Return ONLY valid JSON without any markdown formatting.
      ...
      final result =
          response.text?.trim() ??
          '{"seasons": ["summer"], "colors": ["white", "blue"]}';
...
Return ONLY valid JSON without any markdown block formatting (no ` ```json ` tags, just the raw JSON text).
  ...
  Future<String?> getStyleRecommendations(
    String contextStr,
    List<ClothingItem> wardrobe,
  ) async {
    try {
...
Return ONLY valid JSON without any markdown block formatting (no ` ```json ` tags).
  ...
  Future<String> getStylingAdvice(
...
gently suggest what to change.
      ...
      final promptText = prompt
          .replaceFirst('{weather}', weather)
          .replaceFirst('{items}', itemDescriptions);
...
Be encouraging and professional.
      ...
      final content = [
        Content.multi([
          TextPart(prompt.replaceFirst('{items}', itemDescriptions)),
          imagePart,
        ]),
      ];
  ...
  /// AI Judge to compare two outfits
  Future<String> compareOutfits(
    ...
    try {
      if (_textApiKey == 'PLACEHOLDER_GEMINI_KEY') {
        return "I vote Option Alpha. The layout feels more cohesive and works well for $eventContext.";
      }

      final alphaFile = File(alphaImagePath);
      final betaFile = File(betaImagePath);

      if (!await alphaFile.exists() || !await betaFile.exists()) {
        return "Please make sure both outfits are selected to compare.";
      }
...
Please evaluate both options and give me a clear, professional recommendation.
CRITICAL: Analyze how each outfit handles the weather/temperature mentioned. 
- Is it breathable enough for heat? 
- Is it warm enough for cold? 
- Is it appropriate for the specific event?

Provide a clear choice (Alpha or Beta) and 2-3 sentences of justification focusing on both Style and Weather suitability.
Keep your response concise and friendly. (Max 4 sentences total).
      ''';

      final alphaPart = DataPart('image/jpeg', alphaBytes);
      final betaPart = DataPart('image/jpeg', betaBytes);
      final content = [
        Content.multi([TextPart(prompt), alphaPart, betaPart]),
      ];

      final response = await _visionModel.generateContent(content);
      return response.text ?? "Both outfits look great, you can't go wrong!";
    } catch (e) {
      print('Error comparing outfits: $e');
      return "Both outfits have their own charm. Choose the one that makes you feel most confident!"; // fallback
    }
  }
}
